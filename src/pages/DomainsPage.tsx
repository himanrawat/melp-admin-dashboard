...
const mockDomains: Domain[] = [
  { id: "1", domain: "melp.com", type: "primary", status: "verified", users: 156, addedAt: "2023-01-01" },
  { id: "2", domain: "getmelp.com", type: "alias", status: "verified", users: 0, addedAt: "2023-06-15" },
  { id: "3", domain: "app.melp.com", type: "subdomain", status: "verified", users: 45, addedAt: "2023-03-10" },
  { id: "4", domain: "staging.melp.com", type: "subdomain", status: "verified", users: 24, addedAt: "2023-04-05" },
  { id: "5", domain: "newdomain.com", type: "alias", status: "pending", users: 0, addedAt: "2026-03-20", dnsRecord: { type: "TXT", name: "_melp-verify.newdomain.com", value: "melp-verification=abc123def456ghi789" } },
  { id: "6", domain: "olddomain.com", type: "alias", status: "failed", users: 0, addedAt: "2026-03-01", dnsRecord: { type: "TXT", name: "_melp-verify.olddomain.com", value: "melp-verification=xyz987uvw654rst321" } },
]
...
function AddDomainDialog({ open, onClose, onAdd }: {
  open: boolean
  onClose: () => void
  onAdd: (domain: string, type: Domain["type"]) => void
}) {
  const [domain, setDomain] = useState("")
  const [type, setType] = useState<Domain["type"]>("alias")

  function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!domain) return
    onAdd(domain, type)
    setDomain(""); setType("alias")
    onClose()
  }
      ...
      <DialogContent>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-2">
          <div className="flex flex-col gap-1.5">
            ...
            <Input placeholder="e.g. yourdomain.com" value={domain} onChange={(e) => setDomain(e.target.value)} />
          </div>
          <div className="flex flex-col gap-1.5">
            <Label>Domain Type</Label>
            <div className="flex gap-2">
              {(["primary", "alias", "subdomain"] as const).map((t) => (
                <button
                  key={t}
                  type="button"
                  onClick={() => setType(t)}
                  className={`flex-1 py-2 px-3 rounded-lg border text-sm font-medium transition-colors capitalize ${
                    type === t ? "border-foreground bg-secondary" : "border-border hover:bg-secondary/50"
                  }`}
                >
                  {t}
                </button>
          ...
          <div className="flex flex-col gap-2 p-3 rounded-lg bg-secondary/50 text-xs text-muted-foreground">
            ...
            <p>1. Copy the DNS verification record</p>
            <p>2. Add it to your DNS provider</p>
            <p>3. Wait for propagation (up to 24h)</p>
          </div>
          <DialogFooter className="mt-2">
            <Button type="button" variant="outline" onClick={onClose}>Cancel</Button>
            <Button type="submit" className="melp-radius">Add Domain</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
    ...
    <div className="mt-3 p-3 rounded-lg border bg-secondary/30 text-xs">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="flex justify-between items-start gap-2">
          ...
          <span className="font-mono text-right break-all">{record.name}</span>
        </div>
        <div className="flex justify-between items-start gap-2">
          <span className="text-muted-foreground shrink-0">Value</span>
          <div className="flex items-center gap-1">
            <span className="font-mono text-right break-all">{record.value}</span>
            <button onClick={() => copy(record.value)} className="shrink-0 text-muted-foreground hover:text-foreground">
              {copied ? <IconShieldCheck className="size-3.5 text-success" /> : <IconCopy className="size-3.5" />}
            </button>
          </div>
        </div>
      </div>
...
export function DomainsPage() {
  ...
  function handleAdd(domain: string, type: Domain["type"]) {
    const newDomain: Domain = {
      ...
      dnsRecord: {
        type: "TXT",
        name: `_melp-verify.${domain}`,
        value: `melp-verification=${Math.random().toString(36).slice(2, 18)}`,
      },
    }
  ...
  return (
    <div className="flex flex-1 flex-col gap-4 p-4 lg:p-6 overflow-y-auto overflow-x-hidden">
        ...
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            <div className="flex items-center justify-center size-9 rounded-lg bg-success/10 shrink-0">
              <IconShieldCheck className="size-4 text-success" />
            </div>
            <div>
        ...
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold">{domains.reduce((a, d) => a + d.users, 0)}</p>
            </div>
          </CardContent>
        </Card>
      </div>
        ...
        <div className="flex gap-1.5">
          {(["all", "verified", "pending", "failed"] as const).map((s) => (
            <Button
              ...
              size="sm"
              className={statusFilter === s ? "melp-radius" : ""}
      ...
      <div className="flex flex-col gap-3">
        {filtered.length === 0 ? (
          <p className="text-center text-muted-foreground py-12">No domains found.</p>
        ) : filtered.map((d) => {
          const sConfig = statusConfig[d.status]
          const tConfig = typeConfig[d.type]
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-start gap-3 flex-1">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <Badge variant="secondary" className={`text-xs flex items-center gap-1 ${sConfig.className}`}>
                          <SIcon className="size-3" />
                          {sConfig.label}
                        </Badge>
                      </div>
                      <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
                        <span>{d.users} users</span>
                        <Separator orientation="vertical" className="h-3" />
                        <span>Added {d.addedAt}</span>
                      </div>
                      {d.dnsRecord && d.status !== "verified" && (
                        <DnsRecordCard record={d.dnsRecord} />
                      )}
                      {d.status === "failed" && (
                        <div className="flex items-center gap-2 mt-2 text-xs text-destructive">
                          <IconAlertTriangle className="size-3.5 shrink-0" />
                          Verification failed. Check your DNS record and try again.
                        </div>
                      )}
                    </div>
                  </div>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      {d.status !== "verified" && (
                        <DropdownMenuItem>
                          <IconRefresh className="size-4 mr-2" /> Retry Verification
                        </DropdownMenuItem>
                      )}
                      {d.dnsRecord && (
                        <DropdownMenuItem>
                          <IconCopy className="size-4 mr-2" /> Copy DNS Record
                        </DropdownMenuItem>
                      )}
                      <DropdownMenuSeparator />
                      <DropdownMenuItem
                      ...
                      >
                        <IconTrash className="size-4 mr-2" /> Remove Domain
                      </DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </div>
